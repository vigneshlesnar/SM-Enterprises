     ...
     if (isset($_POST['submit']) && isset($_FILES['pro_image'])) {
        ...
        if ($error === 0) {
            ...
            if (in_array($img_ex_lc, $allowed_exs)) {
                ...
                $sql="UPDATE `product` SET `pro_image`='$new_img_name',`category`='$category',`pro_name`='$name',`price`='$price' WHERE `id`='$id' ";
                $res = $conn -> query($sql);
                if($res===TRUE)   
                {
                        $check= "Updated";
			...
			<div class="card z-depth-3 hoverable grey lighten-4"><br>
				...
				<div class="container"><?php echo $check; ?>
					...
					<form method="POST" action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']);?>" enctype="multipart/form-data"><br><br>
						<input name="pro_image" placeholder="Image" id="blog_image" type="file" class="validate grey lighten-4">
						<br><br>
						<input name="category" placeholder="Imagetext" id="text_image" type="text" class="validate grey lighten-4" value="<?php echo $category;?>">
						<br><br>
                        <input name="name" placeholder="Imagetext" id="text_image" type="text" class="validate grey lighten-4" value="<?php echo $name;?>">
						<br><br>
                        <input name="price" placeholder="Imagetext" id="text_image" type="text" class="validate grey lighten-4" value="<?php echo $price;?>">
						<br><br>
